...
Usage:
    arbitr8der status [--json]
    arbitr8der snapshot [--json]
    arbitr8der health
    arbitr8der markets
    arbitr8der predict
    arbitr8der vessel status
    arbitr8der vessel battery
    arbitr8der vessel forward
    arbitr8der vessel stop
...
def snapshot(json_output: bool = typer.Option(False, "--json", help="Output as JSON")) -> None:
    ...
    from kalshi_desk_package.data_sources.coinbase_spot_price_stream import CoinbaseSpotPriceStream
    from kalshi_desk_package.data_sources.coingecko_macro_data_poller import CoinGeckoMacroDataPoller
...
def health() -> None:
    ...
    typer.echo("")
    typer.echo("  Run 'arbitr8der vessel battery' to start data collection mode.")
...
def predict(
    ...
    from kalshi_desk_package.durable_storage.sqlite_database_engine_manager import initialize_database
...
def forward_start(
    ...
    from kalshi_desk_package.cli.interactive_trading_repl_loop import start_repl
