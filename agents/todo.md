...
- **Root cause of silent paper-autotrader diagnosed (2-part).** (a) All 130 logged decisions in the 2026-09-01 1-hour session were `skip_reason=no_kalshi_midpoint` because the Kalshi book arrived only via the authenticated WebSocket, which 401s after key rotation. (b) **Even with a public REST fallback wired, the Kalshi 15m BTC/ETH markets expose NO resting orderbook** in this environment: for the current-cycle market sampled repeatedly (incl. 44s before close), `markets/{ticker}` returns `yes_bid=None`/`yes_ask=None` and `/orderbook` returns 0 yes + 0 no levels. A control sweep of 100 generic `status=open` markets returned 0 with a bid/ask. So there is genuinely no midpoint for this series to consume — the absence is Kalshi liquidity/supply, not a parsing or auth bug.
- **Fix (pushed `ab5a8b2`):** added a public REST orderbook fallback (`get_orderbook_snapshot` on the discovery client + a 5s `kalshi-rest-orderbook` poller feeding the snapshot merger) so `kalshi_midpoint_cents` would stay populated for paper trading without auth. That wiring is correct — a live 70s validation confirmed fresh REST book events merged every 5s (`book_ts` advancing) — but the target book is empty, so `yes_bid/ask` and midpoint stayed `None`. Added `test_kalshi_rest_orderbook.py`. Full suite: **415 passed, 3 skipped**.
- **Discovery hardened (pushed `3628c83`):** `discover_active_markets` now sorts discovered BTC/ETH 15m markets by close time so the nearest-expiry (currently-live) market is targeted instead of a stale/far market; added `KalshiMarketDetail.close_epoch`. Also added a 120s auto-trader status print (trades/skips/recent decision reasons) to `run_ai_trading_session.py`. Verified live: discovery targets the correct current-cycle market.
...
- **Next action (decision):** do NOT launch another 1-hour paper session yet — it would only re-log `no_kalshi_midpoint` for 60 min (confirmed no Kalshi midpoint exists for KXBTC15M right now). Before the next session, resolve the liquidity path: (1) obtain a valid production Kalshi API key + matching RSA PEM so the authenticated WS streams the real full book (the intended source); or (2) confirm whether a liquid cohort of Kalshi markets should be targeted (the 15m crypto series currently has no exposed book), and re-point discovery/venue accordingly. Rust scaffold of `kalshi_desk/rust_kalshi_vessel/` remains pending.
