"""CWD-independent path resolution for trading_studio runtime directories.
...
when launching the studio from the repo root, from `trading_studio/`, or from
pytest running out of `tests/`.
"""

from pathlib import Path

# trading_studio/ is the package root (parent of arbitr8der_package/)
...
def get_package_root() -> Path:
    """Return the absolute path to the trading_studio/ directory."""
    return _PACKAGE_ROOT


def get_repository_root() -> Path:
    """Return the absolute path to the ARBITR8DER/ directory (parent of trading_studio/)."""
...
def resolve_streams_path(relative_or_absolute: str) -> Path:
    ...
    Relative inputs are resolved against ``trading_studio/streams/`` (not CWD).
